...
import { ChevronLeft, ChevronRight, Undo2, Redo2, HelpCircle, Keyboard, Save, PanelRightOpen, PanelRightClose, Share2, Image, FileText, Type, Table2, PenTool, Eye, Mail, Globe, Lock } from 'lucide-react';
...
import { publishDocument } from '@/lib/api';
...
export default function DocumentEditor() {
  ...
  const startAtPreview = searchParams.get('step') === 'preview';
  ...
  const { status: saveStatus, markDirty, save: autoSaveTrigger, isDirty } = useAutoSave(saveFunc);
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        // Jump to preview if ?step=preview
        if (startAtPreview) {
          const steps = isSuratResmi(found as DocumentData) ? SURAT_RESMI_STEPS : ALL_STEPS;
  ...
  }, [doc, autoSaveTrigger, t]);

  const handleTogglePublish = useCallback(async () => {
    if (!doc || !id) return;
    setPublishLoading(true);
    try {
      const newVal = !(doc as any).isPublished;
      await publishDocument(id, newVal);
      setDoc(prev => prev ? { ...prev, isPublished: newVal } as any : prev);
      toast({ title: newVal ? '✓ Dokumen dipublikasikan ke semua user' : 'Dokumen dikembalikan ke privat', duration: 2000 });
    } catch (err: any) {
      toast({ title: 'Gagal mengubah status publikasi', description: err.message, variant: 'destructive' });
    } finally {
    ...
    <div className="flex flex-col min-h-screen bg-background">
      ...
      <div className="sticky top-0 z-40 flex items-center justify-between border-b bg-background/95 backdrop-blur px-4 py-2">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <SaveIndicator status={saveStatus} onClick={async () => {
            await autoSaveTrigger();
            toast({ title: `✓ ${t('editor.saved')}`, duration: 1500 });
          }} />
          <div className="h-4 w-px bg-border" />
          <TooltipProvider>
            ...
            {user?.role === 'admin' && (
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button
                    variant={(doc as any).isPublished ? 'default' : 'outline'}
                    size="sm"
                    className={`gap-1.5 text-xs h-8 ${(doc as any).isPublished ? 'bg-emerald-600 hover:bg-emerald-700 text-white border-0' : ''}`}
                    onClick={handleTogglePublish}
                    disabled={publishLoading}
                  >
                    {(doc as any).isPublished ? <Globe className="h-3.5 w-3.5" /> : <Lock className="h-3.5 w-3.5" />}
                    <span className="hidden sm:inline">{(doc as any).isPublished ? 'Dipublikasikan' : 'Publikasikan'}</span>
                  </Button>
                </TooltipTrigger>
                <TooltipContent>{(doc as any).isPublished ? 'Klik untuk jadikan privat' : 'Publikasikan ke semua user'}</TooltipContent>
              </Tooltip>
            )}
            <Tooltip>
              <TooltipTrigger asChild>
            ...
            <Button
              ...
              onClick={async () => {
                try {
                  ...
                  toast({ title: 'Gagal menyimpan template', description: err.message || 'Coba lagi', variant: 'destructive' });
                }
              }}
            >
              Simpan

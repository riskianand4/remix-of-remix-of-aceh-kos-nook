...
/* ─── Gate Screen ─── */
function GateScreen({ code, onAccess }: { code: string; onAccess: (doc: DocumentData, shareCode: string, comments: CommentApi[]) => void }) {
  ...
  const handleAccess = async () => {
    ...
    try {
      ...
      onAccess(result.document, result.shareCode, cmts);
    } catch (err: any) {
      setError(err.message || 'Link tidak ditemukan atau kode akses salah');
    } finally {
      setLoading(false);
    }
  };
...
export default function ReviewPage() {
  ...
  const handleAccessSuccess = (d: DocumentData, sc: string, cmts: CommentApi[]) => {
    setDoc(d); setShareCode(sc); setComments(cmts); setPhase('review');
  };
  ...
  const handleResolve = async (comment: CommentApi) => {
    try {

...
function formatKopText(raw: string, textColor: string): string {
  const lines = raw.split("\n").filter((l) => l.trim());
  if (lines.length === 0) return "";
  const first = `<strong style="font-size:14pt;display:block;margin-bottom:1px;color:${textColor};">${escapeHtml(lines[0])}</strong>`;
  const rest = lines
    .slice(1)
    .map((l) => escapeHtml(l))
    .join("<br/>");
  return first + (rest ? `<span style="font-size:11pt;color:${textColor};">${rest}</span>` : "");
}

function renderIconContent(svgContent: string, textColor: string): string {
  const trimmed = svgContent.trim();
  if (trimmed.startsWith("<svg")) {
    // Inline SVG — inject color and size
    const sized = trimmed
      .replace(/width="[^"]*"/, "")
      .replace(/height="[^"]*"/, "")
      .replace(/class="[^"]*"/, "")
      .replace("<svg", `<svg width="14" height="14" style="vertical-align:middle;margin-right:3px;color:${textColor};"`);
    return sized;
  }
  // Treat as image URL
  return `<img src="${escapeHtml(trimmed)}" style="height:14px;width:14px;vertical-align:middle;margin-right:3px;" />`;
}

function buildKopIconsHtml(icons: { svgUrl: string; text: string }[], textColor: string): string {
  const validIcons = icons.filter(ic => ic.svgUrl || ic.text);
  if (validIcons.length === 0) return "";
  const items = validIcons.map(ic => {
    const iconTag = ic.svgUrl ? renderIconContent(ic.svgUrl, textColor) : "";
    return `<span style="display:inline-flex;align-items:center;margin-right:12px;font-size:8pt;color:${textColor};">${iconTag}${escapeHtml(ic.text)}</span>`;
  }).join("");
  return `<div style="margin-top:2px;">${items}</div>`;
}
...
function generateSuratResmiHtml(doc: DocumentData, qrDataUrl: string): string {
  ...
  const kopDividerHtml = kopDividerEnabled ? `<hr class="kop-divider" style="border-color:${kopDividerColor}; border-radius:"10px"; />` : "";
  const kopIconsHtml = buildKopIconsHtml(kopIcons, kopTextColor);
  const kopBlock = hasKop
    ? `
    <div class="kop-top-wrapper">
      <div class="kop-header" style="display:flex; align-items:center; gap:16px;">
        ${doc.kopLogoDataUrl ? `<img src="${doc.kopLogoDataUrl}" class="kop-logo" style="flex-shrink:0;" />` : ""}
        ${
          doc.kopText
            ? `
          <div class="kop-text" style="flex:1; color:${kopTextColor}; ${kopLogoPosition === "center" ? "text-align:center;" : "text-align:left;"}">
            ${formatKopText(doc.kopText, kopTextColor)}
            ${kopIconsHtml}
          </div>
        `
            : ""
...
<style>
  ...
  * { margin: 0; padding: 0; box-sizing: border-box; }
  ...
  .image-page .kop-top-wrapper ~ .image-page-content,
  .image-page-content { flex: 1; display: flex; flex-direction: column; min-height: 0; }
  .image-page-content .image-grid { flex: 1; min-height: 0; }
  ...
  .kop-text { font-size: 10pt; line-height: 1.5; }
  .kop-divider { border: none; border-top: 3px solid ${kopDividerColor}; margin-bottom: 0; }
  .kop-top-wrapper {
    position: absolute;
    top: 0;
    left: 0;
  ...
  @media print {
    .kop-top-wrapper { padding: 0; }
    .kop-bottom-wrapper { padding: 0; }
  }
...
function generateDokumentasiHtml(doc: DocumentData, qrDataUrl: string): string {
  ...
  const pageSize =
    orientation === "landscape" ? `${paperSize} landscape` : paperSize;
  const theme = buildTheme(doc.customTheme);
  const m = doc.margins || { top: 15, bottom: 15, left: 15, right: 15 };
  ...
  // 2. Desain KOP Atas: KOP -> Garis -> 1mm spacing (fixed)
  const kopTop =
    hasKop && (kopPosition === "top" || kopPosition === "both")
      ? `
    <div class="kop-top-wrapper">
      <div class="kop-header" style="display:flex; align-items:center; gap:10px; padding-bottom:4px;">
        ${kopContent}
      </div>
      ${kopDividerEnabled ? `<hr class="kop-divider" style="margin-bottom:0;border-color:${kopDividerColor};" />` : ""}
    </div>
    <div style="margin-bottom:1mm;"></div>
  `
      : "";

  // 3. Desain KOP Bawah: Divider di ATAS teks, fixed di bottom
  const kopBottom =
    hasKop && (kopPosition === "bottom" || kopPosition === "both")
      ? `
    <div class="kop-bottom-wrapper">
      ${kopDividerEnabled ? `<hr class="kop-divider" style="margin-bottom:0px; margin-top:0;border-color:${kopDividerColor}; border-radius:3px;" />` : ""}
      <div class="kop-header" style="display:flex; align-items:center; gap:10px; padding-top:4px;">
        ${kopContent}
      </div>
    </div>
  `
  ...
  doc.sections.forEach((section) => {
    ...
    for (let i = 0; i < section.images.length; i += imagesPerPage) {
      ...
      pages.push(`
        <div class="page image-page" data-section="content">
          ${watermarkHtml}
          ${kopTop}
          <div class="image-page-content">
            ${i === 0 && section.title.trim() ? `<h2 class="section-title" style="margin:0 0 4px 0;">${escapeHtml(section.title)}</h2>` : ""}
      ...
      if (pages.length > 0) {
        ...
        if (insertPoint > -1) {
          pages[pages.length - 1] =
            lastPage.slice(0, insertPoint) +
            sigContent +
            lastPage.slice(insertPoint);
        } else {
          pages.push(
            `<div class="page" data-section="signatures">${watermarkHtml}${kopTop}${sigContent}${kopBottom}</div>`,
          );
        }
      } else {
        pages.push(
  ...
  // Inject QR code on last page only
  if (qrDataUrl && doc.qrEnabled !== false && pages.length > 0) {
    const qrHtml = `
      <div class="qr-last-page">
        <img src="${qrDataUrl}" alt="QR Verification" />
        <p class="teks-visibility">Scan untuk verifikasi</p>
        <p class="teks-visibility" style="font-size:5pt;">${escapeHtml(doc.docCode)}</p>
      </div>
    `;
    const lastIdx = pages.length - 1;
    const lastPage = pages[lastIdx];
    const closeDiv = lastPage.lastIndexOf("</div>");
    if (closeDiv > -1) {
      pages[lastIdx] = lastPage.slice(0, closeDiv) + qrHtml + lastPage.slice(closeDiv);
    }
  }
...
@media print {
    .page { 
      padding: 0; 
      /* RUMUS PUPPETEER: 100vh dikurangi total margin atas dan bawah */
      min-height: calc(100vh - ${m.top}mm - ${m.bottom}mm - 2px); 
      position: relative;
      display: block;
      page-break-after: always;
      overflow: visible;
  ...
  .kop-text { font-size: 10pt; line-height: 1.5; }
  .kop-divider { border: none; border-top: 9px solid ${kopDividerColor}; margin-bottom: 0; box-shadow: 0 4px 6px rgba(0,0,0,0.5); }
  .kop-top-wrapper {
    position: absolute;
    top: 0;
    left: 0;
  ...
  @media print {
    .kop-top-wrapper { padding: 0; top: 0; }
    .kop-bottom-wrapper { padding: 0; bottom: -115px; }
    .qr-last-page{
     bottom: -115px;
    }
  }
  .qr-last-page {
    position: absolute;
    bottom: ${(doc.kopPosition === 'bottom' || doc.kopPosition === 'both') && hasKop ? '30mm' : '12mm'};
    left: ${m.left}mm;
    z-index: 20;
    text-align: center;
  }
  ...
  .qr-last-page img { width: 50px; height: 50px; }
  .qr-last-page p { font-size: 6pt; color: #666; margin-top: 2px; }
  ...
  /* Image Grids */
  .image-grid { display: grid; gap: 8px; position: relative; z-index: 1; break-inside: avoid; width: 100%; }
  .grid-1x1 { grid-template-columns: 1fr; }
  .grid-1x2 { grid-template-columns: 1fr; grid-template-rows: 1fr 1fr; height: calc(100% - 20px); }
  .grid-2x2 { grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr; height: calc(100% - 20px); }
  .grid-2x3 { grid-template-columns: 1fr 1fr; grid-template-rows: 1fr 1fr 1fr; height: calc(100% - 20px); }

  .image-item { break-inside: avoid; text-align: center; overflow: hidden; display: flex; flex-direction: column; min-height: 0; }
  .grid-image { width: 100%; flex: 1; min-height: 0; object-fit: cover; border: 2px solid ${theme.dividerColor}; border-radius: 4px;}
  .grid-1x1 .grid-image { aspect-ratio: 3/4; }
  .grid-1x2 .grid-image { aspect-ratio: 15/9; }
  .grid-2x2 .grid-image { aspect-ratio: 4/5; }
  .grid-2x3 .grid-image { aspect-ratio: 16/13; }
  .image-caption { font-family: ${theme.fontFamily}; font-size: 9pt; font-weight: bold; color: ${theme.bodyColor}; margin-top: 2px; flex-shrink: 0; line-height: 1.2; }
